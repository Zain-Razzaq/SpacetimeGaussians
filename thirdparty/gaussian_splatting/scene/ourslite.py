    ...
    def addgaussians(self, baduvidx, viewpoint_cam, depthmap, gt_image, numperay=3, ratioend=2, trbfcenter=0.5,depthmax=None,shuffle=False):
        ...
        for zscale in ratiaolist :
            ...
            if shuffle == True:
                ...
                targetPz = (depths + depths/10*(randomdepth)) *zscale # adding some noice to dept map
            else:
                targetPz = depths*zscale # depth in local cameras..
            ...
            # converts the 4D homogeneous coordinate back into a 3D Cartesian coordinate.
            # directioninlocal will contain the points in Cartesian coordinates (3D space) after the division by the w component.
...
#            new_trbf_center.append(torch.rand(1).cuda() * torch.ones((Npoints, 1), device="cuda")) 
            ...
            new_trbf_center.append(torch.ones((selectnumpoints, 1)).cuda()
                                    * viewpoint_cam.timestamp)

            # new_trbf_center.append(torch.rand((selectnumpoints, 1)).cuda())
            # give same time to all gaussian as of current time of the view point
